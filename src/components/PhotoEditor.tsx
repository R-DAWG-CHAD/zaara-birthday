...
const STICKERS = [
  { id: 'glasses', name: 'Pink Shades', src: '/stickers/sunglasses.jpg' },
  { id: 'crown', name: 'Gold Crown', src: '/stickers/crown.jpg' },
  { id: 'flower', name: 'Floral Bouquet', src: '/stickers/flowers.jpg' }
];
...
export default function PhotoEditor({ photos, mode, onComplete, onCancel }: PhotoEditorProps) {
  ...
  const [activeStickers, setActiveStickers] = useState<{id: string, src: string, key: number}[]>([]);
  const [isProcessing, setIsProcessing] = useState(false);
  const captureRef = useRef<HTMLDivElement>(null);
  const [stickerCounter, setStickerCounter] = useState(0);

  const addSticker = (sticker: typeof STICKERS[0]) => {
    setActiveStickers([...activeStickers, { id: sticker.id, src: sticker.src, key: stickerCounter }]);
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 gap-4">
            {STICKERS.map(s => (
              ...
              >
                <img src={s.src} alt={s.name} className="h-16 object-contain mix-blend-multiply" />
          ...
          <div className="absolute inset-0 z-20 pointer-events-none">
            {activeStickers.map(sticker => (
              <motion.img 
                ...
                drag 
                dragMomentum={false}
                className="absolute w-40 object-contain cursor-grab active:cursor-grabbing pointer-events-auto mix-blend-multiply" 
